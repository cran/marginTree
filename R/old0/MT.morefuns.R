...
compute.marg.matrix=function(x,y){
k=length(table(y))
...
print.MTgetnonzero <-function(x, ...) {
   cat("Call:\n")

   m=length(x$feature.scores)
for( i in m:1){
    s1=NULL
    for(ii in 1:length(x$left.features[[i]])){
         s1=paste(s1,x$left.features[[i]][ii])
    }
    s2=NULL
    for(ii in 1:length(x$right.features[[i]])){
         s2=paste(s2,x$right.features[[i]][ii])
    }

   titl=paste("Split: classes ", s1," vs " , s2,sep="")
